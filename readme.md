...
```mermaid
flowchart TD
    A[ESP32 wakes up] --> B[Request sleep duration]
    B --> C[Get next wake time from cron]
    C --> D[Request image via MAC]
    D --> E[Get Frame's Group]
    E --> F[Select Active Prompt]
    F --> G[Get Random Image]
    G --> H[Return Image to ESP32]
    H --> I[ESP32 displays image]
